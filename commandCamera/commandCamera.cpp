// commandCamera.cpp : 定义控制台应用程序的入口点。
//
//
	...
	while (n < argc)
	{
		...
		else if (strcmp(argv[n], "/filename") == 0)
		{
			...
			if (++n < argc) //strcpy(filename, argv[n]);
			{
				...
				strcpy_s(char_buffer, argv[n]);

				// Trim inverted commas if present and copy
				// provided string into filename char array
				if (char_buffer[0] == '"')
				{
					strncat_s(filename, char_buffer, strlen(char_buffer) - 2);
				}
				else
				{
					strcpy_s(filename, char_buffer);
				}
			}
		...
		else if (strcmp(argv[n], "/devname") == 0)
		{
			...
			if (++n < argc)
			{
				...
				if (char_buffer[0] == '"')
				{
					strncat_s(device_name, char_buffer, strlen(char_buffer) - 2);
				}
				else
				{
					strcpy_s(device_name, char_buffer);
				}
	...
	while (1)
	{
		...
		if (device_number == 0)
		{
			...
			if (hr == S_OK)
			{
				...
				sprintf_s(char_buffer, "%ls", var.bstrVal);
				VariantClear(&var);
				pPropBag->Release();
				pPropBag = NULL;
