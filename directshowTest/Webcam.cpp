...
public:
	...
	void ErrMsg(LPTSTR szFormat, ...)
	{
		...
		MessageBox(hParentsWnd, szBuffer, NULL,
	...
	void openCamera(HWND parentWnd)
	{
		...
		if (1)
		{
			...
			pVW->put_Owner((OAHWND)hParentsWnd);    // We own the window now
			pVW->put_WindowStyle(WS_CHILD);    // you are now a child

			// give the preview window all our space but where the status bar is
			GetClientRect(hParentsWnd, &rc);
			int cyBorder,cy;
